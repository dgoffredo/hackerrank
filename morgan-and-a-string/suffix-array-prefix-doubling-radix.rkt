...
; same "prefix doubling" algorithm used in "suffix-array-refix-doubling.rkt",
; except that a radix sort is used instead of a comparison-based sort (in an
; attempt to speed things up). Thus, the algorithm uses `O(n * log n)` time
; and `O(n)` space.

(require "sorts.rkt"    ; vector-radix-sort!
         "numeric.rkt") ; integer->vector
...
; Note that the choice of `size` in this function assumes input size.
(define (suffix->vector arg)
  "Return a vector suitable for use in radix sorting suffixes by rank only."
  (match arg
    [(suffix rank-high rank-low _)
     (let ([size 4])
       (vector-append 
         (integer->vector rank-high size)
         (integer->vector rank-low size)))]))

(define (suffix-array string
                      alphabet-min
                      alphabet-max
                      #:with-inverse [with-inverse #f])
  "Return a vector that is the suffix array of `string`, where `string`
   consists of characters between `alphabet-min` and `alphabet-max`. If
   `with-inverse`, then return a pair containing the suffix array and
   another vector that maps every string index to the index in the suffix
   array of the suffix beginning at that character in the string, e.g. 
  ...
  ; TODO: The use of the alphabet is all screwed up now.
  (match n
    ...
    [_         ; otherwise...
     (define (string-at index)
       (if (< index n)
         ...
         (sub1 (char->integer alphabet-min)))) ; less than any value: "end"
     ...
     (for ([prefix-length (in-list lengths)])
       ...
       ; would be past the end, then rank-low is zero (which is less than any
       ; rank-high).
         ...
         (set-suffix-rank-low! current-suffix
           (let ([next-string-index
             ...
             (if (>= next-string-index n)
               0 ; the next suffix is beyond the end of the string
