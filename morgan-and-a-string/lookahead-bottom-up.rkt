...
(define (common-prefix-length-table left right)
  ...
  ; Extra row and column to avoid corner cases in the loop below. All zeros.
  (define table (matrix (add1 L) (add1 R) 0))
    ...
    (match-let* ([(list i j) coordinates]
      ...
      (when (char=? left-char right-char)
        (matrix-set! table i j
          (add1 (table (add1 i) (add1 j)))))))

    table)

(define (in-minimal-merge left right)

  (define common-prefix-length (common-prefix-length-table left right))
